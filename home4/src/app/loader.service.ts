...
export class Project {
  ...
  title: string;
  subtitle: string;
  pics: { [name: string]: string };
  description?: string;
  descriptionHtml?: string;
  active: string;
  link: string;
  pubs: string[];
  publications: (CuratedPub|CrossRefItem)[];
}
...
export class LoaderService {
  ...
  getStudents(): Promise< Student[] > {
    return this.http.get('assets/students.json').toPromise().then(response => {
      const rj = response.json();
      console.log('RESPONSE rj ', rj);
      return rj && rj.students;
    }).then((items) => {
      return items.map( i => _.extend(new Student(), i));
    });
  }

  @memoize((x) => x)
  getProjects(): Promise< Project[] > {
    return this.http.get('assets/projects.json').toPromise().then(response => {
      const rj = response.json();
      return rj && rj.projects;
    }).then((items) => {
      return Promise.all<Project>(items.map( i => {
          const p = new Project();
          _.extend(p, i);
          if (p.descriptionHtml && !p.description) {
            return this.http.get(p.descriptionHtml).toPromise().then(resp => {
              p.description = resp.text();
              return p;
            });
          }
         return p;
    ...
    // now return resolved ids
    return this.getMergedPubs().then(pubs => {
      items.map(proj => {
        proj.publications = (proj.pubs || []).map(pubid => pubs[pubid]);
      });
      return items;
    });
  });
